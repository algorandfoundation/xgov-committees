 ...
 */
export async function runUseCache(fromBlock: number, toBlock: number): Promise<void> {
  ...
  const candidateCommittee = await loadCandidateCommittee(fromBlock, toBlock, 's3');
