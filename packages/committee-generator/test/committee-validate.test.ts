...
describe('Committee validator', () => {
  ...
  it('Should fail with committee with whitespace committee', () => {
    ...
    expect(() => validateCommitteeString(str)).toThrow(/Committee JSON included whitespace/);
  });
  ...
  it('Should fail with out of order xGovs', () => {
    ...
    expect(() => validateCommitteeString(JSON.stringify(unsortedXgovs))).toThrow(
      /Committee JSON xGov array was not sorted/,
    );
  });
  ...
  it('Should fail with invalid address', () => {
    ...
    expect(() => validateCommitteeString(JSON.stringify(testCommittee))).toThrow(
      `Committee JSON included invalid address: ${invalid}`,
    );
  });
  ...
  it('Should fail without extra keys', () => {
    const testCommittee = getCommitteeFixture();
    expect(() => validateCommitteeString(JSON.stringify({ ...testCommittee, extra: 1 }))).toThrow(
      `Committee JSON did not pass schema validation: must NOT have additional properties`,
    );
  });

  it('Should fail with []', () => {
    expect(() => validateCommitteeString(JSON.stringify([]))).toThrow(
